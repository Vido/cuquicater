...
terraform {
  backend "s3" {
    ...
    #bucket = "#####(replace) tfstate bucket name. flamboyant-terraform-state#####"
    bucket = "ckltftest-terraform-state"
    # Replace this with your DynamoDB table name!
    #dynamodb_table = "#####(replace) tflocks table name. flamboyant-terraform-locks#####"
